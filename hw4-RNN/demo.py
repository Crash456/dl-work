...
# 数据预处理（加载word2vec、建立词典、构建矩阵、句子转索引、长度对齐、标签转tensor）
class Preprocess:
    # 初始化
    ...
    # 获取训练好的 word2vec 的 word embedding
    def get_w2v_model(self):
        # 读取之前训练好的 word2vec
        self.embedding = Word2Vec.load(self.w2v_path)
        self.embedding_dim = self.embedding.vector_size

    # 给特殊 token（<PAD> 和 <UNK>）加 embedding
...
def training(batch_size, n_epoch, lr, train, valid, model, device):
    ...
    for epoch in range(n_epoch):
        ...
        for i, (inputs, labels) in enumerate(train):
            ...
            )  # 因为 device 为 "cuda"，将 inputs 转成 torch.cuda.LongTensor
            labels = labels.to(
                device, dtype=torch.float
            )  # 因为 device 为 "cuda"，将 labels 转成 torch.cuda.FloatTensor，loss()需要float

            optimizer.zero_grad()  # 由于 loss.backward() 的 gradient 会累加，所以每一个 batch 后需要归零
            outputs = model(inputs)  # 模型输入Input，输出output
            outputs = (
                outputs.squeeze()
            )  # 去掉最外面的 dimension，好让 outputs 可以丢进 loss()
        ...
        with torch.no_grad():
            ...
            for i, (inputs, labels) in enumerate(valid):
                ...
                accuracy = evaluation(
                    outputs, labels
                )  # 计算模型此时的 training accuracy
                total_acc += accuracy / batch_size
                total_loss += batch_loss.item()

            print(
                "Valid | Loss:{:.5f} Acc: {:.3f} ".format(
            ...
            if total_acc > best_acc:
                ...
                best_acc = total_acc
                torch.save(model.state_dict(), "ckpt_weights.pth")
        print("-----------------------------------------------")
...
model = model.to(
    device
)  # device为 "cuda"，model 使用 GPU 来训练（inputs 也需要是 cuda tensor）
...
# 读取测试数据test_x
print("loading testing data ...")
model.load_state_dict(torch.load("ckpt_weights.pth", map_location=device))  # 加载权重
test_x = load_testing_data("testing_data.txt")
# 对test_x作预处理
...
# 读取模型
print("\nload model ...")
model.load_state_dict(torch.load("ckpt_weights.pth", map_location=device))
# 测试模型
